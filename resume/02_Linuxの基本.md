...
<br />
## 確認問題
1) `ls` コマンドのオプションを調べてリストを縦一列に表示してください。

2) `cd` コマンドで自分のユーザーディレクトリに一発で戻ってください。

3) `mkdir` コマンドにオプションをつけて、今自分がいるディレクトリに `test1/test2/test3` ディレクトリを作ってください。

4) `touch` コマンドで `test1.txt` を作成してください。
